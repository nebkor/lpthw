...
def rewind(f):
	f.seek(0) # using the .seek() method on a file object to (is this the right way to say this?)
			  # return the read state of the file back to the 0th byte
...
def print_a_line(line_count, f):
	print line_count, f.readline(), # prints the raw value of line_count followed by a space
									...
									# to print a string from the file object starting at the current
									# byte(?) until the next new line
...
# current_file variable, which performs the .read() method from the object's
# current state to the end of the file.
...
# setting the file state (again,hmm, must be a better way) 
rewind(current_file)
